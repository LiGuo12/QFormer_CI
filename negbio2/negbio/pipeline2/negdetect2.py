    ...
    def __call__(self, doc, *args, **kwargs):
        try:
            for passage in doc.passages:
                # print(f"\nProcessing passage with {len(passage.annotations)} annotations")
                ...
                # print(f"Found {len(locs)} locations to check")
                ...
                # print(f"Created map for {len(sentence_locs_map)} sentences")
                ...
                for sentence, sublocs in sentence_locs_map.items():
                    ...
                    #     _mark_anns(passage.annotations, loc[0], loc[1], name,
                    #                self.detector.total_patterns[matcher.pattern])
                    # print(f"\nChecking sentence: {sentence.text}")
                    # print(f"Checking {len(sublocs)} locations")
                    for name, matcher, loc in self.detector.detect(sentence, sublocs):
                        # print(f"Found {name} at position {loc}")
