...
class RunnerBase:
            ...
            if self.use_distributed:
                if self._wrapped_model is None:
                    self._wrapped_model = DDP(
                        self._model, device_ids=[self.config.run_cfg.gpu], find_unused_parameters=True,broadcast_buffers=False)
    ...
    @torch.no_grad()
    def extract_features(self, cur_epoch="best"):
        """Extract features from training set because we need to cluster them later"""
        ...
        logging.info(f"Expected number of batches: {total_batches}")
        ...
        # 
        for batch_idx, batch in enumerate(itertools.islice(data_loader, total_batches)):
            image_features = model.extract_features(batch)
            logging.info(f"Processing batch {batch_idx + 1}/{total_batches}, feature shape: {image_features.shape}")
            
            with h5py.File(feature_file, 'a') as f:
                for feat in image_features:
                    f['features'].create_dataset(
                        str(global_idx),
                        data=feat.cpu().numpy().astype(np.float32),
                        dtype='float32',
                        compression="gzip"
                    )
                    global_idx += 1
        ...
        logging.info(f"Total number of features extracted: {global_idx}")
        logging.info(f"Total number of batches processed: {batch_idx + 1}")
        
        # verify features num
        with h5py.File(feature_file, 'r') as f:
            num_features = len(f['features'])
            first_feat = f['features']['0'][()]
            logging.info(f"Number of features in h5 file: {num_features}")
            logging.info(f"Feature dtype: {first_feat.dtype}") 
                    ...
                    if val_log is not None:
                        if is_main_process():
                            ...
                            if agg_metrics > best_agg_metric and split_name == "val":
                                logging.info(f"epoch {cur_epoch} is the best and the agg_metirc is {agg_metrics}")
