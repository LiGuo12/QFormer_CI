    ...
    def forward(self, x):
        ...
        if self.fused_attn:
            with torch.no_grad():
                ...
                self.attn_weights = (q_scaled @ k.transpose(-2, -1)).softmax(dim=-1)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        self.last_attn = self.attn.attn_weights
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x):
        ...
        for blk in self.blocks:
            x = blk(x)
            attentions.append(blk.last_attn)
        # x = self.norm(x)
        return x, attentions
    ...
    def forward(self, imgs):
        latent, attn = self.forward_encoder(imgs)
        return latent, attn
...
def create_medical_mae_vit_b(img_size=224,precision="fp16", vit_ckp_path=None):
    ...
    if vit_ckp_path is not None:
        ...
        ckp_dict = torch.load(vit_ckp_path, map_location="cpu")
    else:
        raise FileNotFoundError(f"Image encoder checkpoint not found at {vit_ckp_path}")
    state_dict = ckp_dict['model']
    # Process and load weights 
