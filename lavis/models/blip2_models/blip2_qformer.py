...
from lavis.models.blip_models.blip_outputs import BlipOutput, BlipOutputFeatures, ExtendedBlipOutput, BlipLLMOutput, Blip2Output
...
@registry.register_model("multiview_blip2")
class MultiViewBlip2QFormer(Blip2Base):
    """
    Multi-view version of BLIP2 QFormer that handles both single-view and multi-view inputs.
    Inherits from the original Blip2Qformer to maintain all original functionalities.
    """
    PRETRAINED_MODEL_CONFIG_DICT = {
        "pretrain": "configs/models/blip2/blip2_pretrain.yaml",
        "pretrain_vitL": "configs/models/blip2/blip2_pretrain_vitL.yaml",
    ...
    def evaluate(self, samples):
        ...
        # feature_file = "D:/code/ubc/paper/new_paper/code/image_features_medical_mae_vit/features.h5"
        feature_file = "/scratch/st-zjanew-1/li/codes/image_features_medical_mae_vit/features.h5"
        ...
        if len(image.shape) == 5:  # Multi-view input: [B, V, C, H, W]
            ...
            #     image_ids = samples.get("image_id", [str(i) for i in range(32)]) # batch size 32
                
            #     # ######
            #     with h5py.File(feature_file, 'a') as f:
            #         for i in range(32):
        ...
        # Get predicted text
        pred_tokens = torch.argmax(lm_output.logits, dim=-1)
        
        # Decode predicted and true text
        predicted_captions = self.tokenizer.batch_decode(pred_tokens, skip_special_tokens=True)
        gt_captions = self.tokenizer.batch_decode(text_tokens.input_ids, skip_special_tokens=True)
    ...
    def generate(
        ...
        # image_embeds = self.ln_vision(self.visual_encoder(image)) # [B, V*(num_patches+1), hidden_size]
...
@registry.register_model("blip2_llm")
class Blip2QFormerLLM(Blip2Base):
    """
    """
    PRETRAINED_MODEL_CONFIG_DICT = {
        "pretrain": "configs/models/blip2/blip2_pretrain.yaml",
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast():
            llm_outputs = self.opt_model(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
                return_dict=True,
                labels=targets,
            )
        loss_llm = llm_outputs.loss

        return BlipLLMOutput(
            # loss=loss_itc + loss_itm + 0.5 * loss_lm + 0.5 * loss_llm,
            loss=loss_itc + loss_itm + loss_llm,
            loss_itc=loss_itc,
            loss_itm=loss_itm,
            # loss_lm=loss_lm,
            loss_llm=loss_llm,
    ...
    def generate(
        ...
        return ExtendedBlipOutput(
            # Q_former_predicted_captions=captions,
            Q_former_predicted_captions = output_text,
